...
import static com.splunk.tracer.shared.AbstractTracer.InternalLogLevel.DEBUG;
import static com.splunk.tracer.shared.AbstractTracer.InternalLogLevel.ERROR;
import static com.splunk.tracer.shared.Options.VERBOSITY_DEBUG;
import static com.splunk.tracer.shared.Options.VERBOSITY_FIRST_ERROR_ONLY;
import static com.splunk.tracer.shared.Options.VERBOSITY_INFO;
...
public abstract class AbstractTracer implements Tracer, Closeable {
    ...
    protected static final String SPLUNK_TRACER_PLATFORM_KEY = "tracer_platform";

    @SuppressWarnings("unused")
    protected static final String SPLUNK_TRACER_PLATFORM_VERSION_KEY = "tracer_platform_version";

    @SuppressWarnings("unused")
    protected static final String SPLUNK_TRACER_VERSION_KEY = "tracer_version";
    ...
    private void maybeStartReporting() {
        ...
        if (metaEventLoggingEnabled && !firstReportHasRun) {
            buildSpan(SplunkTracingConstants.MetaEvents.TracerCreateOperation)
                    .ignoreActiveSpan()
                    .withTag(SplunkTracingConstants.MetaEvents.MetaEventKey, true)
                    .withTag(SplunkTracingConstants.MetaEvents.TracerGuidKey, reporter.getReporterId())
                    .start()
                    .finish();
            firstReportHasRun = true;
        }
        reportingThread = new Thread(reportingLoop, SplunkTracingConstants.Internal.REPORTING_THREAD_NAME);
    ...
    public Tracer.SpanBuilder buildSpan(String operationName) {
        return new com.splunk.tracer.shared.SpanBuilder(operationName, this);
    }
    ...
    public <C> void inject(io.opentracing.SpanContext spanContext, Format<C> format, C carrier) {
        ...
        SpanContext splunkSpanContext = (SpanContext) spanContext;
        if (metaEventLoggingEnabled) {
            buildSpan(SplunkTracingConstants.MetaEvents.InjectOperation)
                    .ignoreActiveSpan()
                    .withTag(SplunkTracingConstants.MetaEvents.MetaEventKey, true)
                    .withTag(SplunkTracingConstants.MetaEvents.SpanIdKey, splunkSpanContext.getSpanId())
                    .withTag(SplunkTracingConstants.MetaEvents.TraceIdKey, splunkSpanContext.getTraceId())
                    .withTag(SplunkTracingConstants.MetaEvents.PropagationFormatKey, format.getClass().getName())
                    .start()
                    .finish();
        }
        ...
        propagator.inject(splunkSpanContext, carrier);
    }
    ...
    public <C> io.opentracing.SpanContext extract(Format<C> format, C carrier) {
        ...
        if (metaEventLoggingEnabled) {
            buildSpan(SplunkTracingConstants.MetaEvents.ExtractOperation)
                    .ignoreActiveSpan()
                    .withTag(SplunkTracingConstants.MetaEvents.MetaEventKey, true)
                    .withTag(SplunkTracingConstants.MetaEvents.PropagationFormatKey, format.getClass().getName())
                    .start()
